...
void RenderCallBase::Render()
{
	...
	if (vertices != nullptr && vertices->count() != 0)
	{
		...
		const char* ptr = vertices->_vbo != 0 ? nullptr : reinterpret_cast<const char*>(vertices->data());
		...
		glDrawArrays(vertices->_mode, 0, vertices->count());
		CHECK_ERROR_GL();
