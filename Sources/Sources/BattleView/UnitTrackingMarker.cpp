...
void UnitTrackingMarker::RenderTrackingMarker(TextureBillboardRenderer* renderer)
{
	...
	{
		glm::vec2 destination = DestinationXXX();
		glm::vec3 position = _battleModel->groundMap->GetHeightMap()->GetPosition(destination, 0);
...
void UnitTrackingMarker::RenderOrientation(GradientTriangleRenderer* renderer)
{
	if (_renderOrientation && _hasOrientation && !_path.empty())
	{
		...
		renderer->AddVertex(_battleModel->groundMap->GetHeightMap()->GetPosition(center + 10.0f * left, 0), glm::vec4(0, 0, 0, 0));
		renderer->AddVertex(_battleModel->groundMap->GetHeightMap()->GetPosition(tip + overshoot * dir, 0), glm::vec4(0, 0, 0, 0.1f));
		renderer->AddVertex(_battleModel->groundMap->GetHeightMap()->GetPosition(center - 10.0f * left, 0), glm::vec4(0, 0, 0, 0));
	}
}
